...
void F0_for_line1(void) { 
    bool defer = False  || ( (((executionStack[3].count) > (0)) || ((executionStack[0].count) < (executionStack[1].count))) || ((executionStack[1].count) < (executionStack[2].count)));
    if(!defer){
        bool again = False;
        // Starting statements 
        printf("%llu", executionStack[0].count);
...
void F1_for_line2(void) { 
    bool defer = False  || ( ((executionStack[3].count) > (0)) || ((executionStack[0].count) == (executionStack[1].count)));
    if(!defer){
        bool again = False;
        // Starting statements 
        printf("%s", "Take one down and pass it around,");
        printf("\n");
        // Statements done
...
void F2_for_line3(void) { 
    bool defer = False  || ( ((executionStack[3].count) > (0)) || ((executionStack[1].count) == (executionStack[2].count)));
    if(!defer){
        bool again = False;
        // Starting statements 
        printf("%llu", executionStack[0].count);
...
void F3_for_line4(void) { 
    ...
    if(!defer){
        bool again = False;
        // Starting statements 
        change_pos_quantity(0,98);
        change_pos_quantity(1,98);
        change_pos_quantity(2,98);
        // Statements done
